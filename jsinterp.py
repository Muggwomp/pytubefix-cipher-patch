...
def extract_player_js_global_var(jscode):
    ...
    # Some TCE players assign the large string table to a namespaced property
    # (for example g.O=[...]) well after the "use strict" prologue. The nsig
    # resolver still needs this table to find the _w8_ anchor.
    w8_pos = jscode.find("_w8_")
    if w8_pos == -1:
        return None, None, None

    window_start = max(0, w8_pos - 100000)
    assignment_pattern = re.compile(
        r'(?P<code>(?P<name>(?:[a-zA-Z0-9_$]+\.)?[a-zA-Z0-9_$]+)\s*=\s*\[)'
    )
    matches = list(assignment_pattern.finditer(jscode, window_start, w8_pos))
    for match in reversed(matches):
        array_start = match.end() - 1
        depth = 0
        quote = None
        escaped = False
        for idx in range(array_start, len(jscode)):
            char = jscode[idx]
            if quote:
                if escaped:
                    escaped = False
            ...
            if char in ("'", '"'):
                ...
                depth -= 1
                if depth == 0:
                    value = jscode[array_start:idx + 1]
                    if "_w8_" in value:
                        return (
                            jscode[match.start():idx + 1],
                            match.group("name"),
                            value,
                        )
                    break
